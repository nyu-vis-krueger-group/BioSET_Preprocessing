#!/usr/bin/env python
"""
STAGE 1: GPU tile processing with checkpointing.

This script processes tiles and saves checkpoints. It can be interrupted
and resumed - just run it again and it will skip completed tiles.
...
print("\n" + "="*60)
print("STAGE 1: GPU Tile Processing (with checkpointing)")
print("="*60)

start_time = time.time()
pipe = Pipeline(cfg)
tiles_processed = pipe.run_tile_processing(resume=True)
elapsed = time.time() - start_time

print(f"\n{'='*60}")
print(f"GPU STAGE COMPLETE")
print(f"Tiles processed: {tiles_processed}")
print(f"Time: {elapsed/60:.1f} minutes")
print(f"{'='*60}")
print("\nNext: Run 'python run_cpu.py' to aggregate and create database")